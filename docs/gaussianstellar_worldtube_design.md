# GaussianStellar Worldtube Design

## Goal

Move GaussianStellar beyond:
...
- `GaussianStellar worldtube` treats time as part of the primitive support itself.
...
- GaussianStellar is geometry-first: semantics are attached after a stronger spacetime base exists.
...
- In GaussianStellar, TRASE is treated as a reusable semantic layer, not the geometric core.
...
- Semantic transfer from TRASE to early GaussianStellar HyperNeRF smoke runs is still weak because the current 300-iteration tube checkpoint does not yet produce stable entity-aligned tracks.
