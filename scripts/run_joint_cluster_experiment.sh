...
EXPERIMENT_ROOT="${GS_EXPERIMENT_ROOT:-${GS_ROOT}/experiments}"
STAMP="${GS_EXPERIMENT_STAMP:-$(date -u +%Y%m%d_%H%M%S)}"

case "${SCENE_KEY}" in
  split-cookie)
    RUN_DIR="${GS_ROOT}/runs/stellar_worldtube_split-cookie_compare5k/hypernerf/split-cookie"
    DATASET_DIR="${GS_ROOT}/data/hypernerf/misc/split-cookie"
    ...
    ;;
  cut-lemon1)
    RUN_DIR="${GS_ROOT}/runs/stellar_worldtube_cut-lemon1_quality5k/hypernerf/cut-lemon1"
    DATASET_DIR="${GS_ROOT}/data/hypernerf/interp/cut-lemon1"
