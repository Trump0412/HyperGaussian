...
def build_report() -> str:
    ...
    query_rows = {
        ...
        "GaussianStellar-native-v1": load_query_summary(
            REPO_ROOT
            / "runs/stellar_worldtube_cut_lemon1_smoke300_v6a/hypernerf/cut-lemon1/entitybank/native_queries/gaussianstellar_native_cuts_the_lemon",
            REPO_ROOT
            / "runs/stellar_worldtube_cut_lemon1_smoke300_v6a/hypernerf/cut-lemon1/entitybank/native_queries/gaussianstellar_native_cuts_the_lemon/rendered/validation.json",
            bridge_active,
            bridge_roles,
        ),
        "GaussianStellar-native-v5": load_query_summary(
            REPO_ROOT
            / "runs/stellar_worldtube_cut_lemon1_smoke300_v6a/hypernerf/cut-lemon1/entitybank/native_queries/gaussianstellar_native_cuts_the_lemon_v5",
            REPO_ROOT
            / "runs/stellar_worldtube_cut_lemon1_smoke300_v6a/hypernerf/cut-lemon1/entitybank/native_queries/gaussianstellar_native_cuts_the_lemon_v5/rendered/validation.json",
            bridge_active,
            bridge_roles,
        ),
    }
    ...
    query_table_rows = []
    for label in ["TRASE-bridge", "GaussianStellar-native-v1", "GaussianStellar-native-v5"]:
    ...
    semantic_rows = [
        ...
        [
            ...
            "前者走外部强语义后端，后者走 GaussianStellar 自身语义打分",
            "当前 bridge 更稳，native 还在追",
        ],
    ]

    lines: list[str] = []
    lines.append("# GaussianStellar 架构与 Pipeline 汇报稿")
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "GaussianStellar 的目标不是只做“更好的时间编码”，而是把动态场景从 `time-conditioned 3D Gaussian` 推进到 `spacetime-native Gaussian primitive`："
    )
    lines.append("")
    ...
    lines.append("   - 查询层既支持 `TRASE bridge`，也支持 `GaussianStellar native query`。")
    lines.append("")
    lines.append("## 4. DA3 在系统里的角色")
    lines.append("")
    lines.append("DA3 在 GaussianStellar 里是 `geometry bootstrap`，不是最终动态模型。")
    lines.append("")
    lines.append("实际接法如下：")
    lines.append("")
    ...
    lines.append("4. 后续训练就继续走原来的 4DGS / GaussianStellar 训练入口，因此 DA3 只替换初始化，不替换后续 scene-specific 4D 优化。")
    lines.append("")
    lines.append("这套设计的意义是：")
    lines.append("")
    lines.append("- 保留 GaussianStellar 对时空 primitive、densify/prune、entitybank 的控制权。")
    ...
    lines.append("- `stellar`：`StellarMetricWarp`，是当前 GaussianStellar 常用的 context-aware warp。")
    lines.append("")
    ...
    lines.append("")
    lines.append("## 6. GaussianStellar worldtube 与 vanilla 4DGS 的核心区别")
    lines.append("")
    lines.append(
        table(
            ["维度", "vanilla 4DGS", "GaussianStellar worldtube"],
            [
    ...
    lines.append("- 这说明 GaussianStellar 的主线不是凭空跳到 worldtube，而是先完成 `time reparameterization -> local temporal state -> primitive-level temporal support` 的逐级推进。")
    lines.append("")
    ...
    lines.append("- 这说明 DA3 并没有替代 GaussianStellar；它只是把初始几何变强，而真正的提升仍来自后续时空 primitive 和优化。")
    lines.append("")
    ...
    lines.append("- `GaussianStellar-native-v1` 能大致覆盖事件，但时间窗口过宽，active IoU 只有 `0.4824`。")
    lines.append("- `GaussianStellar-native-v5` 的窗口又收得过晚，active IoU 降到 `0.0000`。")
    lines.append("- 因此，当前最佳实践仍然是：`几何用 GaussianStellar worldtube，语义查询先走 TRASE bridge`。")
    lines.append("")
    lines.append("## 语义架构")
    lines.append("")
    ...
    lines.append("")
    lines.append("1. GaussianStellar 的核心不是“多一个 time MLP”，而是把时间写进 Gaussian primitive 本身。")
    lines.append("2. DA3 在系统里只负责 bootstrap 几何，把弱点云初始化替换成更强的 `fused.ply`；真正的动态建模仍然由 GaussianStellar 完成。")
    ...
    lines.append("- `gaussian_stellar/entitybank/tube_bank.py`")
    ...
    source_paths = [
        ...
        bridge_query_dir,
        REPO_ROOT / "runs/stellar_worldtube_cut_lemon1_smoke300_v6a/hypernerf/cut-lemon1/entitybank/native_queries/gaussianstellar_native_cuts_the_lemon",
        REPO_ROOT / "runs/stellar_worldtube_cut_lemon1_smoke300_v6a/hypernerf/cut-lemon1/entitybank/native_queries/gaussianstellar_native_cuts_the_lemon_v5",
    ]
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default=str(REPO_ROOT / "reports/gaussianstellar_architecture_pipeline_report.md"),
    )
