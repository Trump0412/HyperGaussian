...
from gaussian_stellar.entitybank.tube_bank import load_gaussian_state
...
def _native_texts(
    ...
    global_desc = (
        f"GaussianStellar entity {entity_id}: {color_words} {shape_words} "
...
def export_native_semantic_assignments(
    ...
    for entity in entities_payload.get("entities", []):
        ...
        if assignment is not None:
            ...
            entity_copy["native_semantic_source"] = "gaussianstellar_native"
    ...
    _write_json(
        entitybank_dir / "entities_semantic_native.json",
        {
            **entities_payload,
            ...
            "semantic_source": "gaussianstellar_native",
        },
    )
    return output_path
